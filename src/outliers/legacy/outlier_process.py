...
def insert_process(tabel_name, df_final):
    engine = create_engine(
        "postgresql://postgres:Passw0rd!!@192.168.3.222:5432/EH_SEWER"
    )
    try:
        # 데이터프레임을 DB 테이블에 삽입
...
)  # exist_ok=True로 설정하면 디렉토리가 이미 존재해도 에러를 발생시키지 않음
...
file_handler = RotatingFileHandler(
    log_file_name, maxBytes=max_file_size, backupCount=backup_count
)  ## 파일 핸들러 생성
        ...
        for col in value[1]:
            ...
            if filtered_data.iloc[-1] == 0.0:
                ...
                if last_valid_index is not None:
                    last_valid_value = filtered_data.loc[
                        last_valid_index
                    ]  # 마지막 유효 인덱스의 값

                    # 마지막 0인 값을 이전 값으로 대체
                    # mask를 사용하여 직접적으로 수정
                    chg_df.loc[mask, col] = chg_df.loc[mask, col].replace(
                        0.0, last_valid_value, regex=True
                    )
            else:
                ...
                consecutive_zeros = (
                    (except_now == 0.0).astype(int).rolling(window=2).sum()
                )
                if (consecutive_zeros > 1).any():
                    ...
                    query_dates_list = chg_df.loc[
                        chg_df.index.intersection(zero_indices), "meas_dtm"
                    ].tolist()

                    # query_dates_list에서 날짜를 문자열로 변환
                    query_dates_str = ", ".join(
                        [
                            f"'{date.strftime('%Y-%m-%d %H:%M:%S')}'"
                            for date in query_dates_list
                        ]
                    )
                    ...
                    merged_df = chg_df.merge(
                        replace_df,
                        on=["meas_dtm", "rcs_id", "data_typ"],
                        suffixes=("", "_replace"),
                        how="left",
                    )

                    # 0인 데이터 대체
                    chg_df.loc[mask & (chg_df[col] == 0.0), col] = merged_df.loc[
                        mask & (chg_df[col] == 0.0), f"{col}_replace"
                    ]
...
def main():
    ...
    for i in range(len(rcs_list)):
        for j in range(len(tag_list[i])):
            df.loc[
                (df["rcs_id"] == rcs_list[i]) & (df["data_typ"] == group_list[i]),
                tag_list[i][j],
            ] = df.loc[
                (df["rcs_id"] == rcs_list[i]) & (df["data_typ"] == group_list[i]),
                tag_list[i][j],
            ].clip(
                lower=minmaxValue[i][j][1], upper=minmaxValue[i][j][2]
            )
